...
function getUppercaseNames(items) {
  return items.map((item) => {return item.name.toUpperCase();});
}
...
function getItemPriceByName(items, name) {
  const found = items.find((item) => {return item.name === name;});
  return found.price;
}
...
function getItemsByCategory(items, category) {
  return items.filter((item) => {return item.category === category;});
}
...
function countItems(items) {
  return items.reduce((accumulator, item) => {return accumulator + item.quantity;}, 0);
}
...
function calculateTotalPrice(items) {
  return items.reduce((accumulator, item) => {return accumulator + (item.quantity * item.price);}, 0);
}
